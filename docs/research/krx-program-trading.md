...
`daily`가 가용분만 넣으면 다음날 자동 보완된다. **이 "가용분만" 전제를 코드로 강제한다: `daily`는 앵커=오늘
행의 프로그램 순매수대금(`net_value`)이 null이면(미공표) 그 행을 드롭해 오늘 파티션을 쓰지 않고 다음 세션
자가치유에 맡긴다** — 미공표 잠정 행이 T+1 15:10 판단에 T-1로 유입되는 것을 막는다. **코스닥 초기 관측:
196170 2016년 부근 프로그램 필드 전부 0**(acml_vol은 정상 — 당시 코스닥 프로그램 미미가 원인 후보, 진위
미확정), 2026-07 앵커는 30행 전부 정상.
...
- 15:10 판단에는 항상 T-1 이하만 쓴다(시장단위 18:00 공표, 종목별 당일분은 자가치유 대상). 종목별 당일
  행은 순매수대금 미공표(null)면 `daily`가 드롭하므로 잠정치가 오늘 파티션으로 새지 않는다(§3).
...
- **종목별**: ~2,800종목 × ~87콜(30세션 창 워크백으로 2005~2026 커버) ≈ **24.3만 콜**. `parallel_fetch`
  8워커, 심볼 단위 워크백(30행 미만이면 종목 종료). **심볼 단위 재개는 저장된 최소일 기준**: 최소일이
  `start` 이하면 완전 스킵, 그 외에는 워크백 앵커를 최소일 직전 세션으로 잡아 이미 적재된 구간을 재조회하지
  않는다 — 이미 자연 절벽까지 백필된 종목은 절벽 아래 빈 페이지 1콜만 확인하고 종료하므로, 절벽이 `start`보다
  높아 재실행이 전 종목을 처음부터 다시 워크백하는 예산 낭비를 막는다. **구현만 완료 — 미실행, 사용자 지시 대기.**
...
| KIS 종목 | `program_stock_1d` | (symbol,) | 심볼 워크백, 최소일 기준 재개 | 앵커=오늘, 미공표 오늘 행 드롭, day 그룹 1회 upsert | 전체만, 수정 스케일 |
