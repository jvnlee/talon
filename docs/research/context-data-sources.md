...
2. **분봉 거래량은 KRX+NXT 합산이고 08:00~ 프리마켓 봉이 포함된다** — 일봉 거래량(KRX 단독)과 직접 비율을 내면 안 된다. "당일 거래량 / 20일 평균 거래량" 같은 팩터를 분봉 합산으로 계산하면 1.4~1.8배 부풀어 보인다. 분봉↔일봉을 섞는 팩터는 반드시 같은 계열끼리 비교할 것.
...
#### 1.5-A-2 해법 후보 3안 (Phase 2 종가베팅 착수 전 결정 필요)

이 문제는 **KRX Open API로 못 푼다** — 일별 데이터만 주고 장중·실시간이 없다. `close_strength`의 진입 게이트는 15:10~15:30에 `거래량 ≥ Ref(Mean(volume,20),1) × 2.5`를 봐야 하는데, 그 시각엔 일봉이 없어(EOD는 16:40) 당일 거래량은 토스 분봉 합산뿐이다. 1.4배 부푼 값을 KRX 기준 20일 평균에 대면 **실제 1.77배밖에 안 터진 종목이 2.5배로 보인다**(2.5 ÷ 1.41). 백테스트는 양쪽 다 KRX 일봉이라 내부적으로 앞뒤가 맞아 **이 왜곡을 절대 못 잡는다 — 라이브에서만 조용히 터진다.**

| 안 | 내용 | 대가 | 판정 |
|---|---|---|---|
| **① 같은 계열끼리 비교** | 라이브 판단을 전부 토스 계열로 통일 — 20일 평균도 **토스 일봉**으로, 당일 거래량도 토스 분봉으로. 둘 다 NXT 포함이라 비율이 정상화된다. | **백테스트도 토스 일봉으로 다시 돌려야** 라이브와 같은 세계가 된다. 토스 일봉의 과거 깊이가 **미실측**(DESIGN.md 열린 질문). | 확실하지만 비용 큼 |
| **② 거래소 구분되는 시세원** | KIS(한국투자증권) 등 브로커가 NXT 도입 후 거래소 구분(KRX 단독 / NXT / 통합)을 지원하는 정황이 있다. 사실이면 **KRX 단독 장중 거래량**을 직접 받는다. | **KRX 단독 장중 시세를 실제로 주는지 미확인.** 계좌 개설 + 어댑터 자작 필요. | **근본 해결 — 먼저 확인할 것** |
| **③ 비율 보정** | 종목별 NXT 점유율을 추정해 나눈다. | 점유율이 종목·날짜마다 흔들린다. | **비추천** |

**권고: ②를 먼저 실측**하고, 안 되면 ①로 간다.
...
### 데이터 리스크 등록부
- **거래소 이원화(NXT)**: 브로커(토스) 시세 = KRX+NXT 합산·시간외 포함, KRX 공식 시세 = 정규장 KRX 단독 → 두 계열을 섞으면 팩터가 조용히 왜곡되고, 한쪽을 다른 쪽의 검증 기준으로 쓰면 정상 데이터를 버린다(1.5-A 참조). **NXT 메인마켓(09:00:30~15:20)은 KRX 정규장과 동시에 돌기 때문에 시간대 필터로는 못 걷어낸다** — 분봉을 09:00~15:30으로 잘라도 거래량이 여전히 1.41배 부풀어 있다(005930, 2026-07-13 실측: KRX 31,882,652 vs 분봉합산 45,039,764). 토스 분봉 응답에 거래소 구분 필드가 없어 뺄셈으로도 분리 불가.
