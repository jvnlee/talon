...
- **일별 VKOSPI**: 파생지수 화면(MDCSTAT01001, clssCd=0202)의 "코스피 200 변동성지수" 공식 일별 OHLC+등락을 `vkospi_1d`로 적재(스펙·운영은 `docs/research/krx-vkospi-daily.md`). 확정 종가는 15:45 이후 산출이라 15:10 판단에는 항상 T-1.
...
- **일별 VKOSPI KRX 로그인 SPOF**: `vkospi_1d`도 KRX 로그인 스크래핑(파생지수 화면)이라 EOD(pykrx)·pulse 경로와 **단일 장애점을 공유**한다. 원천이 KRX 하나뿐이라 크로스체크 백업이 없다 — KRX가 화면을 막으면 함께 죽는다(`docs/research/krx-vkospi-daily.md` §6).
