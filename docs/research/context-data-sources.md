...
### 데이터 리스크 등록부
- **거래소 이원화(NXT)**: 브로커(토스) 시세 = KRX+NXT 합산·시간외 포함, KRX 공식 시세 = 정규장 KRX 단독 → 두 계열을 섞으면 팩터가 조용히 왜곡되고, 한쪽을 다른 쪽의 검증 기준으로 쓰면 정상 데이터를 버린다(1.5-A 참조).
- **pykrx 사망(구조적)**: KRX 회원제 전환으로 `fetch_daily_ohlcv`가 상시 실패. 당일 일봉은 FDR 전종목 스냅샷이 유일한 실질 경로이므로 FDR가 깨지면 당일 일봉이 통째로 비는 단일 장애점이다.
- **marcap 발행 지연**: 정본 백필 소스인 marcap은 2거래일 안팎 늦게 발행된다(2026-07-13 시점 최신 07-09) → 당일치는 FDR로 메우고, 지연분은 매일 밤 따라잡기 잡(`com.talon.backfill`)이 자동 재시도한다.
- **거래일 캘린더 오차**: `exchange_calendars`(XKRX)는 임시공휴일을 모른다(2026-06-03 지방선거일을 거래일로 오판) → `markets/kr.py`의 `CLOSURES_MISSING_FROM_XKRX`에 수동 등록 필요. 새 임시공휴일이 지정되면 추가할 것.
