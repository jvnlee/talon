...
**[사실 · 실측] 종목기본정보가 유니버스 필터를 정식화해준다 (미구현 · 착수 가치 큼).**
`SECUGRP_NM`(주권 2,718 / 부동산투자회사 23 / 외국주권 12 / 주식예탁증권 9 / …), `KIND_STKCERT_TP_NM`(보통주 2,652 / 구형우선주 78 / 신형우선주 23 / 종류주권 12), **`SECT_TP_NM`에 관리종목 74 · SPAC 71 · 투자주의환기종목 45가 직접 들어있다.**
- 현재 백테스트 유니버스 규칙 `symbol.ends_with("0")`을 KRX 공식 분류와 대조: **잘못 포함 47건**(SK리츠·KB스타리츠·ESR켄달스퀘어리츠 등 리츠 23 + 외국주권 12 + 주식예탁증권 9 + 기타 3), 빠뜨림 0건. → **지금 백테스트는 리츠를 주식으로 착각하고 있다.**
- 관리종목을 FDR `StockListing("KRX-ADMINISTRATIVE")`로 받는 현 구조는 **실패 시 조용히 `None`을 반환해 관리종목이 무경보로 유니버스에 진입**한다. KRX 공식으로 대체하면 이 침묵 실패가 사라진다.

**[미해결] 라이브와 백테스트의 유니버스가 서로 다르다 — 착수 시 여기부터 볼 것.**

| | 라이브 `ingest/universe.py:build_universe` | 백테스트 `quant/universe.py:LiquidityUniverse.filter` |
|---|---|---|
| 거래량 | `> 0` | `> 0` |
| 보통주 판정 | 토스 `security_type=="STOCK" AND is_common_share` | **`symbol.ends_with("0")`** ← 리츠 47종목 오통과 |
| 관리종목 제외 | FDR 관리종목 목록 (실패 시 **무경보 통과**) | **없음** |
| ETF/ETN/SPAC 제외 | 토스 필터에 부분 의존 | **없음** |
| 거래대금 하한 | `≥ 10억` | `≥ min_value` |
| 크기 | 상위 300 + pinned | 상위 300 |

즉 **백테스트가 검증한 유니버스와 라이브가 실제로 매매하는 유니버스가 다르다.** KRX 종목기본정보를 일자별로 적재해 양쪽이 같은 분류를 쓰게 하는 것이 정답. 새 데이터셋(`stock_info` 등)이 필요하고, 유니버스가 바뀌면 **과거 백테스트 성과가 달라진다**(관문 1 결과 재산출 필요).
