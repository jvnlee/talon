...
> **【실측 2026-07-19】** (Q5 해소) 마지막 체결보다 늦은 앵커는 **마지막 실제 봉으로 클램프**된다 — 2026-07-16 대상 앵커 `153000`과 `173000`이 15:30 마감 동시호가봉으로 끝나는 **동일한 120봉**을 반환. 채택 설계: 고정 `LATE_ANCHOR="173000"`(정상일엔 세션종료 앵커와 결과 동일, 수능 +1h 이동일엔 실제 16:30 꼬리까지 포획). XKRX는 2025-11-13 수능 이동 스케줄을 모델링하지 않음(실측: session_open 09:00·session_close 15:30 vs 실제 10:00·16:30) → verify는 세션종료 상한을 `SESSION_SHIFTS` 상수(2025-11-13·2026-11-19 각 +1h)로 보정해 16:30 마감봉을 세션 최종봉으로 인식·검산. **KRX가 매년 수능일을 공표할 때마다 이 상수를 갱신해야 함.**
> **【실측 2026-07-21】** (첫 프로덕션 백필·검산에서 확정) 종가 동시호가 VI 연장봉: 마감 동시호가 중 VI가 발동하면 종가 체결이 2분 늦게 찍혀 마감봉 시각이 15:32(수능 +1h 이동일엔 16:32)로 나옴 — 68M행 중 1,613행 전부 이 시각, 실제 KRX 메커니즘(데이터 정상). verify는 세션종료 상한과 crosscheck 마감봉 인식에 `CLOSE_VI_EXTENSION`(+2분)을 허용(15:30·15:32 둘 다 마감봉으로 인정).
> **【실측 2026-07-21】** 오전 시가필드 잔상: 09:00~09:30 첫 봉에서 `open`만 [low,high] 밖(직전·기준가 잔상으로 추정), high/low/close는 정합 — 68M행 중 26행 전부 저유동 종목 첫 봉. h/l/c는 신뢰하되 첫 봉 `open`은 참고값으로만. verify는 이를 `open_outliers`(정보용 카운터, status 무영향)로 분리하고 `ohlc_violations`(status 결정)는 h/l/c 위반과 `close<=0`만으로 판정.
> **【실측 2026-07-21】** `candles_1d`(일봉)는 기업행위(액면분할·유무상증자 등)로 소급 정정되나 `kis_minutes_1m`은 체결 당시 원가(raw as-traded)를 저장 → 정정일 이전 구간에서 마감봉 종가가 종목별 상수배로 어긋남(첫 실측 4/30종목 비율 17.30·0.5003·0.3385·0.6674, 나머지 26종목은 원 단위까지 일치). crosscheck는 `ADJUST_FACTORS`(종목별 day/factor, factor=정정/원가 비율, 이후 기업행위 없으면 1.0)를 좌조인해 `factor==1.0`이면 정확일치, 아니면 `|minute_close*factor − daily_close|/daily_close > 0.001`로만 불일치 판정.
> **【미검증】** `FID_FAKE_TICK_INCU_YN="Y"`(허봉 포함)의 정확한 의미·행 생성 규칙(현재 기본 "" 무거래분 생략만 사용).
...
- **【실측 2026-07-21】 24h 경계 함정**: 24h를 넘는 장기 백필 도중 API 게이트웨이는 `EGW00123`(토큰 만료)를 반환하는데 **발급 엔드포인트의 시계는 수 분 뒤처져** 아직 유효 판정으로 **동일한 만료 임박 토큰을 그대로 재발급** → 재시도가 즉시 소진되어 3일 연속 실패·중단. 대응(KisClient 구현): ①만료 시각을 메모리 추적하고 `TOKEN_EXPIRY_MARGIN`(10분) 안이면 **선제 재발급**(만료 전에 미리 교체), ②재발급이 이전(stale) 토큰을 돌려주면 `TOKEN_RETRY_WAIT`(60s) 대기 후 `STALE_TOKEN_ATTEMPTS`(8회)까지 재시도해 발급 시계 스큐 창을 **넘겨 다리 놓기**(bridge), ③`get()`의 `EGW00123` 재시도 분기도 5xx·유량 분기처럼 `1.0*(attempt+1)`s 백오프. 실측 규율: 발급·수집 경로 분리(장전 선발급)만으로는 >24h 런에서 부족 — 클라이언트가 능동 갱신해야 함
- 앱키·시크릿·토큰 값은 로그/출력에 절대 기록하지 않음(실측 규율 유지)

### 2.2 유량 제한에 안전한 초당 호출 수
- 유량은 **계정(앱키) 단위**: 실전 **20건/초**, 모의 2건/초 【문서】
- **"20건/초"의 단순 초당 집계 해석은 실측과 불일치** 【실측 2026-07-16】: 확장 스윕(순차 1,806콜, ~1.8콜/s)이 15:28+까지 지속되며 close-auction(15:20~, ~1.8콜/s)과 중첩된 구간에서 **합산 평균 ≈3.6콜/s·동시 in-flight ≤2인데도** 양측에서 `EGW00201` 3연속 거절(백오프 1s/2s 후에도) 5종목 발생(스윕 member·program 각 1, 종가 잡 3). 반면 순차 단독 ~1.9콜/s는 이틀간 수천 콜 무오류 → **검증된 안전 지속률 ≈2콜/s(단독·순차)**. 원인 후보: ①초 미만 하위 창에 페이싱 강제(비조율 2프로세스의 동시착탄이 위반) ②실계정 초당 창이 문서보다 작음 ③누적형(토큰버킷) 제한. 어느 쪽이든 **비조율 다중 프로세스가 금지 패턴**
- **구현(2026-07-17): 전역 페이서** — 모든 KIS 호출은 `~/.talon/locks/kis-pacer.json`(flock 공유 상태) 슬롯을 거쳐 **프로세스·스레드 불문 시작 간격을 1/`kis_rps`(기본 8/s)로 균일화**. `EGW00201` 관측 시 어느 프로세스든 `kis_penalty_seconds`(기본 30s) 동안 전역 `kis_penalty_rps`(기본 2/s, 위 검증 안전값)로 자동 감속 후 복귀. 수집기는 `kis_workers`(기본 8) 스레드 병렬로 RTT 격벽을 제거하고, `captured_at`은 **행별 실제 수신 시점** 스탬프(PIT 정직성). 주의: **2026-07-15·16 적재분의 captured_at은 스윕 시작 시각 단일값**(실제 수신은 최대 +18분) — 2026-07-17 이후 행만 실시각
- 콜 예산: **호가(1.1)·추정수급(1.2)는 종목당 1콜** → 유니버스 순회 시 콜 수 = 종목 수(스로틀 필수). **랭킹(1.3 top30 / 1.4 top100)은 시장당 1콜**로 콜 수 적음
- **15:10 스윕 확장 콜 예산**(거래대금 상위 300 순회): 종목당 1콜 페처 5종 = 호가(1.1)·추정수급(1.2)·체결강도(1.5)·거래원(1.6)·프로그램 종목별(1.7) + **신규 체결강도/거래원/프로그램/외국계추이(1.5·1.6·1.7·1.9) 각 300콜** → 기존 604콜 + 300×4 + 시장종합(1.8) 2콜 ≈ **1,806콜**. 병렬 8/s 기준 ≈3.8분(15:14께 종료, 15:20 종가 잡과 비중첩), close-auction 패스 304콜 ≈40초(5패스 전부 가능), overtime 306콜 ≈40초. 파트 순서는 시간민감도순: 호가→프로그램 시장종합→체결강도→외국계추이→프로그램 종목별→거래원→외국계랭킹→추정수급→외인기관랭킹(뒤 3종은 14:30 슬롯 고정이라 지연 무해)
- 8/s 균일 페이싱의 실서버 수용 여부는 **2026-07-20(월) 15:10 첫 병렬 실행으로 검증**(§4-17). 거절 시 페널티가 자동 방어하며, 상시 발동이면 `kis_rps` 하향(4→2)이 즉시 대응
