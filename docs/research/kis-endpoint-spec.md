...
> **【실측 2026-07-21 · KRX 확정수급 대금/거래량 원천으로 재판정】** 과거일 분봉은 조회 시점의 **현재 주식수 기준으로 소급 재계산된 수정주가·수정거래량**이다(감자 포함 — 로컬 `ADJUST_FACTORS`도 2026-07-21 기준가 브리지부터 감자를 반영하나(backfill-alternatives 부록 C), KIS 배율은 조회 시점 앵커라 재현 불가 결론은 유지). **오직 `acml_tr_pbmn`(누적 거래대금)만이 재계산 없는 절대량**(15:30 누적 시점에서 KRX 일 거래대금과 ~1.2% 이내 일치). 실측 예 007460 2025-07-08: 원 체결가 657원 vs 분봉 종가 11,369원(×17.30, 감자), 거래량은 역배율(마지막 2시간 분봉 거래량 합 ×17.3 = 원 일 거래량 지분과 일치), 거래대금은 KRX 플로우와 ~1% 이내 일치. 무상증자류 종목은 ×0.5003. 반면 로컬 `candles_1d`는 원 체결가(raw as-traded, KRX 플로우 VWAP과 원 단위 일치, 2026-05-08 감자 불연속 ×15.46 보존) → 어제의 `ADJUST_FACTORS` 좌조인·곱 크로스체크는 원리상 무효라 폐기. **소비 규칙**: 이 가격 **레벨**을 호가단위·상하한가·절대가격 로직에 절대 사용 금지. 안전한 용도는 장중 형태(within-day shape)·수익률·상대강도·거래대금 기반 지표. 한 종목 안에서도 이후 기업행위 전에 조회된 행과 후에 조회된 행의 배율이 다를 수 있음(`fetched_at`가 조회 시각을 표시, 전방 daily 잡은 각 날을 그 시점의 현재 기준으로 자연히 저장). verify는 외부 팩터 없이 원 `candles_1d` 대비 **구간 상수 비율(piecewise-constant ratio) 분절**로 크로스체크(마감봉 비율의 구간 스텝은 기업행위 경계로 인정, 다구간 속 길이-1 이탈만 불일치로 카운트, `restated_symbols`로 재계산 종목 수 보고).
...
- **8/s 균일 페이싱 실서버 수용 확인** 【실측 2026-07-20·21】: 15:10 스윕 1,806콜이 이틀 연속 `EGW00201` 0건·페널티 미발동으로 완주 — captured_at 스팬 3.8분(15:10:40→15:14:31)·3.9분(15:10:35→15:14:28), 예측(≈3.8분·15:14께 종료)과 일치. close-auction도 이틀 연속 5/5패스(패스당 300행). 원인 후보 ①(초 미만 창 페이싱 강제) 지지 — `kis_rps` 하향 불필요, 페널티 경로는 예비 방어로 유지(§4-17)
...
17. **전역 균일 페이싱 8/s의 실서버 수용 — 해결** 【실측 2026-07-20·21】: 판독 지표 전부 통과 — `EGW00201` 이틀 합계 0건(intraday-decision.log의 2건은 전부 2026-07-16 수리 전 발생분), 스윕 captured_at 스팬 3.8분/3.9분(예측 ≈4분 일치), close-auction 5/5패스 복원(07-15 3/5·07-16 2/5 대비). §2.2 원인 후보 ①(초 미만 창 페이싱 강제)과 정합 — 균일 간격이면 8/s 수용됨. `kis_rps` 하향 불요, `EGW00201` 페널티 감속은 예비 방어로 유지
