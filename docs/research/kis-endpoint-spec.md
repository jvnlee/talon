...
- **15:10 스윕 확장 콜 예산**(거래대금 상위 300 순회): 종목당 1콜 페처 5종 = 호가(1.1)·추정수급(1.2)·체결강도(1.5)·거래원(1.6)·프로그램 종목별(1.7) + **신규 체결강도/거래원/프로그램/외국계추이(1.5·1.6·1.7·1.9) 각 300콜** → 기존 604콜 + 300×4 + 시장종합(1.8) 2콜 ≈ **1,806콜**
- **실효 처리율은 스로틀(8rps)이 아니라 왕복지연(RTT)이 지배** 【실측 2026-07-15】: 순차 클라이언트에서 콜당 ≈0.53~0.93초(= 스로틀 0.125초 + RTT). 근거: 종가 동시호가 잡의 300콜 패스가 160~280초 소요(15:23·15:27 패스가 지각 40초 초과로 missed). 즉 확장 스윕은 ≈1.8콜/s로 **약 15~17분(15:27께 종료) 예상** — 15:20 종가 잡과 시간대가 겹치나 합산 ≈3~4콜/s로 유량 한도(20/s)에는 여유. 신규 4파트는 전부 당일 누적치+`captured_at` 스탬프라 캡처 지연이 데이터를 오염시키지는 않음. 단축이 필요하면 병렬화(전역 스로틀 공유 필수) 또는 신규 파트 유니버스 축소가 후보 — 2026-07-16 첫 실행의 파트별 `captured_at` 간격으로 실측 후 결정
