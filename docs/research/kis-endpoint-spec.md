...
> **【실측 2026-07-21 · KRX 확정수급 대금/거래량 원천으로 재판정】** 과거일 분봉은 조회 시점의 **현재 주식수 기준으로 소급 재계산된 수정주가·수정거래량**이다(감자 포함 — 로컬 `ADJUST_FACTORS`는 감자를 정정하지 않는 학파라 이 재계산 배율을 재현 불가). **오직 `acml_tr_pbmn`(누적 거래대금)만이 재계산 없는 절대량**(15:30 누적 시점에서 KRX 일 거래대금과 ~1.2% 이내 일치). 실측 예 007460 2025-07-08: 원 체결가 657원 vs 분봉 종가 11,369원(×17.30, 감자), 거래량은 역배율(마지막 2시간 분봉 거래량 합 ×17.3 = 원 일 거래량 지분과 일치), 거래대금은 KRX 플로우와 ~1% 이내 일치. 무상증자류 종목은 ×0.5003. 반면 로컬 `candles_1d`는 원 체결가(raw as-traded, KRX 플로우 VWAP과 원 단위 일치, 2026-05-08 감자 불연속 ×15.46 보존) → 어제의 `ADJUST_FACTORS` 좌조인·곱 크로스체크는 원리상 무효라 폐기. **소비 규칙**: 이 가격 **레벨**을 호가단위·상하한가·절대가격 로직에 절대 사용 금지. 안전한 용도는 장중 형태(within-day shape)·수익률·상대강도·거래대금 기반 지표. 한 종목 안에서도 이후 기업행위 전에 조회된 행과 후에 조회된 행의 배율이 다를 수 있음(`fetched_at`가 조회 시각을 표시, 전방 daily 잡은 각 날을 그 시점의 현재 기준으로 자연히 저장). verify는 외부 팩터 없이 원 `candles_1d` 대비 **구간 상수 비율(piecewise-constant ratio) 분절**로 크로스체크(마감봉 비율의 구간 스텝은 기업행위 경계로 인정, 다구간 속 길이-1 이탈만 불일치로 카운트, `restated_symbols`로 재계산 종목 수 보고).
