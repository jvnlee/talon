...
- 최적화: `ceil(totalCnt/100)` 페이지 도달 또는 **원시 파싱 행(필터 전 `<tr>` 수)**이 100 미만이면
  그 날 페이징 종료. 필터 후 행 수로 세면 만행 페이지에서 한 행만 떨궈도 뒷페이지가 조용히 잘리므로
  원시 행 수로 판정한다. `has_date` 스킵으로 재개.
...
|---|---|---|---|---|
| `dart_filing_times` | `day`(=`selectDate` 목록일) | `rcept_no` | 달력일 순회, `has_date` 스킵 재개, 1초 페이싱, 연속 3일 실패 중단 | 최근 7 거래세션(오늘 포함) 자가치유 |
...
- 전방 수집은 신규 launchd 없이 기존 eod 잡의 `_load_dart_times` 스텝(실패 격리·정직 상태). **DART
  웹은 키·로그인 불필요.**
- **자가치유 구조:** `daily_dart_times`는 오늘 포함 최근 7 거래세션을 매 실행 재수집(덮어쓰기)한다
  (`cal.sessions_between`으로 세션 창을 잡아 달력일 격차에 무관). 저녁 접수분(예: 15:10 이후 접수)이
  그날 오후 실행(~15:35)에서 누락돼도 **다음 거래일 실행이 그 날을 재수집해 자연 보완**한다. eod는
  거래일에만 돌고 전방 수집의 유일 경로이므로, 창을 **달력일이 아니라 거래세션**으로 세야 추석·설
  같은 장기 휴장(달력 11일 격차)의 직전 거래일도 재수집 범위에 남아 결손이 영구화되지 않는다 —
  세션 창이 늦은 정정·추가 접수와 휴장 격차를 함께 흡수한다.
